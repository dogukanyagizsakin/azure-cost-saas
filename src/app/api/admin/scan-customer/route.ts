...
function detectIssues(resource: any, costPerMonth: number) {
  ...
  const issues = []
  const isTestEnv = name.includes('dev') || name.includes('test') || name.includes('staging') || name.includes('qa') || tags.environment?.toLowerCase()?.includes('dev') || tags.environment?.toLowerCase()?.includes('test')
  ...
  if (type.includes('virtualmachines') || type.includes('hybridcompute/machines') || type.includes('microsoft.hybridcompute')) {
    issues.push({ type: 'idle_vm', title: `Makine İnceleme: ${resource.name}`, description: 'Bu makinenin kullanım durumu incelenmelidir.', saving: saving(0.3) })
    if (isTestEnv) issues.push({ type: 'schedule', title: `Gece Kapatma: ${resource.name}`, description: 'Test/dev ortamı mesai dışında otomatik kapatılabilir.', saving: saving(0.65) })
  }
  if (type.includes('disks')) issues.push({ type: 'underused_disk', title: `Bağlı Olmayan Disk: ${resource.name}`, description: 'Bu disk herhangi bir makineye bağlı değil.', saving: saving(0.95) })
  if (type.includes('publicipaddresses')) issues.push({ type: 'orphan_ip', title: `Kullanılmayan Public IP: ${resource.name}`, description: 'Bu Public IP herhangi bir kaynağa atanmamış.', saving: saving(0.95) })
  if (type.includes('snapshots')) issues.push({ type: 'old_snapshot', title: `Eski Snapshot: ${resource.name}`, description: 'Eski snapshot gereksizse silinmesi önerilir.', saving: saving(0.9) })
  if (type.includes('storageaccounts')) issues.push({ type: 'storage_tier', title: `Depolama Katmanı: ${resource.name}`, description: 'Nadiren erişilen veriler Cool/Archive katmanına taşınabilir.', saving: saving(0.5) })
  if (type.includes('microsoft.web/sites') || type.includes('microsoft.web/serverfarms')) issues.push({ type: 'rightsizing', title: `App Service Rightsizing: ${resource.name}`, description: 'App Service planı kullanıma göre küçültülebilir.', saving: saving(0.35) })
  if (type.includes('microsoft.sql') || type.includes('sqlservers') || type.includes('azurearcdata/sql')) issues.push({ type: 'reserved_instance', title: `SQL Reserved Capacity: ${resource.name}`, description: 'Reserved Capacity ile %33 tasarruf edin.', saving: saving(0.33) })
  if (type.includes('microsoft.insights') || type.includes('microsoft.operationalinsights') || type.includes('microsoft.monitor')) issues.push({ type: 'storage_tier', title: `Log Retention Optimizasyonu: ${resource.name}`, description: 'Log saklama süresini kısaltarak depolama maliyetini düşürün.', saving: saving(0.4) })
  if (issues.length === 0) issues.push({ type: 'rightsizing', title: `Kaynak Optimizasyonu: ${resource.name}`, description: `${resource.type} kaynağı için optimizasyon fırsatları değerlendirilebilir.`, saving: saving(0.2) })
  return issues
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!tenant.azure_client_id) return NextResponse.json({ error: 'Azure credentials eksik' }, { status: 400 })
    ...
    const { data: scanLog } = await adminSupabase
      .from('scan_logs')
      .insert({ tenant_id: tenantId, status: 'running', started_at: new Date().toISOString() })
      .select()
      .single()
    ...
    await adminSupabase.from('scan_logs').update({
      ...
      error_message: !costSupported ? 'Cost Management API bu subscription türünde desteklenmiyor' : null,
    }).eq('id', scanLog?.id)
