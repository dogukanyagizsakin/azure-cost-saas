...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
      ...
      <aside className={`${sidebarOpen ? 'w-60' : 'w-16'} bg-gray-900 border-r border-gray-800 flex flex-col transition-all duration-300 flex-shrink-0`}>
        ...
        <div className="p-4 border-b border-gray-800">
          <div className="flex items-center justify-between">
            ...
            {!sidebarOpen && (
              <button onClick={() => setSidebarOpen(true)} className="absolute left-14 top-4 w-5 h-5 bg-gray-800 border border-gray-700 rounded-full flex items-center justify-center text-gray-400 hover:text-white z-10">
        ...
        {sidebarOpen && (
          <div className="px-3 pt-3">
            <div className={`flex items-center gap-2 px-3 py-2 rounded-lg ${isAzureConnected ? 'bg-green-900/20 border border-green-800/30' : 'bg-yellow-900/20 border border-yellow-800/30'}`}>
              <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${isAzureConnected ? 'bg-green-500' : 'bg-yellow-500'}`}
                style={{ animation: 'pulse 2s infinite' }}
              />
              <span className={`text-xs font-medium ${isAzureConnected ? 'text-green-400' : 'text-yellow-400'}`}>
        ...
        <nav className="flex-1 px-3 py-3 space-y-1 overflow-y-auto">
            ...
            return (
              ...
              >
                <div className={`flex-shrink-0 p-1.5 rounded-lg transition-colors ${
                  isActive ? item.bg : 'group-hover:' + item.bg
                } ${item.color}`}>
                  {item.icon}
                </div>
                {sidebarOpen && (
                ...
                {isActive && (
                  <div className={`absolute right-0 top-1/2 -translate-y-1/2 w-0.5 h-6 rounded-l-full ${item.color} bg-current`} />
                )}
              </Link>
            )
          })}
          ...
          {/* Ayraç */}
          {sidebarOpen && <div className="border-t border-gray-800 my-3" />}
          {sidebarOpen && (
          ...
          <button
            ...
            title={!sidebarOpen ? 'Hızlı Tara' : ''}
          >
            <div className="flex-shrink-0 p-1.5 rounded-lg bg-blue-500/10 text-blue-400 group-hover:bg-blue-500/20">
              {scanning ? (
            ...
            {sidebarOpen && <span className="text-sm font-medium">{scanning ? 'Taranıyor...' : 'Hızlı Tara'}</span>}
          </button>
  ...
  title={!sidebarOpen ? 'Yardım' : undefined}
>
            <div className="flex-shrink-0 p-1.5 rounded-lg bg-cyan-500/10 text-cyan-400 group-hover:bg-cyan-500/20">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <Link
            href="/dashboard/settings"
            className="w-full flex items-center gap-3 px-2.5 py-2.5 rounded-xl transition-all text-gray-400 hover:text-white hover:bg-gray-800/70 group"
            title={!sidebarOpen ? 'Geri Bildirim' : ''}
          >
            <div className="flex-shrink-0 p-1.5 rounded-lg bg-pink-500/10 text-pink-400 group-hover:bg-pink-500/20">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="p-3 border-t border-gray-800">
          <div className={`flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-800/70 transition-colors cursor-pointer group ${!sidebarOpen ? 'justify-center' : ''}`}>
