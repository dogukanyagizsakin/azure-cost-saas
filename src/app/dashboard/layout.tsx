...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-gray-950 overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={`${sidebarOpen ? 'w-60' : 'w-16'} bg-gray-900 border-r border-gray-800 flex flex-col transition-all duration-300 flex-shrink-0 relative`}>
          ...
          {navItems.map(item => {
            ...
            return (
              <Link
                ...
                <div className={`flex-shrink-0 p-1.5 rounded-lg transition-colors ${item.color} ${isActive ? item.bg : ''}`}>
                  {item.icon}
                </div>
                {sidebarOpen && (
                  <>
