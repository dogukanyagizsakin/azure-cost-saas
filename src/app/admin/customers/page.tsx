      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 bg-gray-900 border-b border-gray-800 flex items-center justify-between px-6 flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs text-gray-500">{filtered.length} müşteri</span>
          </div>
          <div className="flex items-center gap-3">
            <input
              ...
              className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 w-48"
            />
            <button
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            {[
              { label: 'Toplam', value: customers.length, color: 'text-white' },
              { label: 'Azure Bağlı', value: customers.filter(c => c.azure_connected).length, color: 'text-green-400' },
              { label: 'Toplam Kaynak', value: customers.reduce((s, c) => s + c.resource_count, 0), color: 'text-blue-400' },
              { label: 'Toplam Tasarruf', value: `$${customers.reduce((s, c) => s + c.total_saving, 0).toFixed(0)}`, color: 'text-emerald-400' },
            ].map((card, i) => (
                ...
                <tbody>
                  {filtered.map((customer, i) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${customer.azure_connected ? 'bg-green-900/50 text-green-400' : 'bg-gray-800 text-gray-500'}`}>
                          {customer.azure_connected ? '✓ Bağlı' : '✗ Bağlı Değil'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-white">{customer.resource_count}</td>
                      <td className="px-6 py-4">
                        <span className={customer.recommendation_count > 0 ? 'text-yellow-400' : 'text-gray-500'}>
                      ...
                      <td className="px-6 py-4 text-gray-400 text-xs">
                        {customer.last_scan
                          ? new Date(customer.last_scan).toLocaleDateString('tr-TR', { day: 'numeric', month: 'short' })
                          : '-'}
                      </td>
                      <td className="px-6 py-4">
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="flex gap-3 mt-2">
                  ...
                  <button type="submit" disabled={creating} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2">
                    {creating ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Oluşturuluyor...</> : 'Oluştur'}
                  </button>
                </div>
              </form>
            </motion.div>
          ...
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                {[
                  { label: 'Azure Durumu', value: selectedCustomer.azure_connected ? '✓ Bağlı' : '✗ Bağlı Değil', color: selectedCustomer.azure_connected ? 'text-green-400' : 'text-red-400' },
                  ...
                  { label: 'Aylık Bütçe', value: selectedCustomer.monthly_budget ? `$${selectedCustomer.monthly_budget}` : 'Belirlenmemiş', color: 'text-white' },
                ].map((item, i) => (
              ...
              {selectedCustomer.azure_subscription_id && (
                <div className="p-3 bg-gray-800/50 rounded-xl mb-4">
                  <p className="text-xs text-gray-500 mb-1">Subscription ID</p>
                  <p className="text-xs text-gray-300 font-mono">{selectedCustomer.azure_subscription_id}</p>
                </div>
              )}

              {selectedCustomer.last_scan && (
                <div className="p-3 bg-gray-800/50 rounded-xl mb-4">
                  <p className="text-xs text-gray-500 mb-1">Son Tarama</p>
                  <p className="text-sm text-white">{new Date(selectedCustomer.last_scan).toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' })}</p>
                </div>
              )}

              <button
                onClick={() => setShowDetailModal(false)}
                className="w-full border border-gray-700 text-gray-400 hover:text-white py-2.5 rounded-xl text-sm transition-colors"
              >
                Kapat
              </button>
            </motion.div>
          ...
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
            ...
            >
              ...
              <p className="text-xs text-gray-500 text-center mb-6">
                Tüm kaynaklar, öneriler ve veriler silinecek. Bu işlem geri alınamaz!
              </p>
              <div className="flex gap-3">
                <button onClick={() => setShowDeleteModal(false)} className="flex-1 border border-gray-700 text-gray-400 hover:text-white py-2.5 rounded-xl text-sm transition-colors">
                  İptal
                </button>
